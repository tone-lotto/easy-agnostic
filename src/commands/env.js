...
import { loadSource, serverAllowed, mergeAgentPolicies } from '../source.js';
import { refsIn, shellQuote } from '../util.js';
import { resolveSecret } from '../secrets.js';
import { AGENTS, NEW_AGENTS } from '../agents.js';
...
export function referencedNames(target, root = projectRoot()) {
  if (target !== undefined && !AGENTS.includes(target)) throw new Error(`--target must be ${AGENTS.join(', ')}`);
  const names = new Set();
  ...
  for (const sc of ['user', 'project']) {
    ...
    const policy = sc === 'user' || !s.hasAgents ? user.agents : mergeAgentPolicies(user.agents,s.agents);
    if (target && policy.targets?.[target] === false) continue;
    if (NEW_AGENTS.includes(target) && policy.targets?.[target] !== true) continue;
