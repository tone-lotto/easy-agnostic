...
2. **MCP is the core.** Skills are wired, never translated. Project-root AGENTS.md and CLAUDE.md can be synchronized bidirectionally via `src/instructions.js`: explicit enrollment, a common content-hash baseline, conflicts when both sides differ, private backups, and a per-project lock. A legacy import becomes shared content; additional Claude-only text stays outside a marked shared region and is never copied into AGENTS.md.
3. **Never clobber.** MCP writes go through the 3-way merge in `src/merge.js` (desired vs last-apply snapshot vs native). Entries the tool never wrote are untouched. A native edit since the last apply is a conflict: stop and show it. Only `--prefer source|native` resolves MCP conflicts, explicitly. Instruction sync uses its common hash baseline; only `eag instructions --prefer agents|claude` selects a winner when both shared texts differ.
...
| codex-user | `~/.codex/config.toml` | managed block between `# >>> easy-agnostic managed >>>` / `# <<< easy-agnostic managed <<<` (matched as whole lines: anything but exactly one open and one close is `damaged` and refuses to write) | no `${VAR}` expansion; entries outside the block (hand-written, or another tool's own managed block) are locked: compared, never edited. `read()` also classifies a locked table as `keep` (another tool's marker block, a command inside a macOS `.app` bundle, the ChatGPT internals `node_repl`/`computer-use`/`cua_repl`, or `enabled = false`) and adopt leaves those to Codex instead of sharing them — the first outside tester got ChatGPT.app registered as an MCP server in Claude Code before this existed; a plain hand-written table is adopted with `targets.codex=false` |
