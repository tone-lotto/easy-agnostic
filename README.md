...
- Skills are shared through `~/.agents/skills`: Codex reads it directly, `doctor --fix` links it into `~/.claude/skills`, and `eag adopt skills` moves a skill that only one agent has into it (Claude is left a link). A copy of the same skill under `~/.codex/skills` makes Codex list it twice (`doctor --fix` removes an identical copy); two *different* skills sharing a name are reported and left alone.
