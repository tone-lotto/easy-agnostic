...
- Because every agent expands `${NAME}` from **its own environment**, the value has to be exported where the agent is started — that is what `eval "$(eag env)"` in your shell rc is for, and `eag doctor` warns when a referenced secret resolves from the store but is missing from the shell. (Verified against claude 2.1.x: user scope expands `${VAR}` in `env` and in HTTP headers, including mid-string. If you hit a build where it does not, set `"claude": {"secrets": "literal"}` in `agents.json` to go back to writing resolved values.)
